...
export class ContentManager {
  ...
  get(key: string, locale?: string): string | null {
    ...
    if (!localizedContent) {
      logger.warn('Content not found for locale', { key, locale: selectedLocale });
      return null;
    }
...
export async function loadContentFromJSON(paths: string[]): Promise<Record<string, LocalizedContent>> {
