...
export class StarsService {
  ...
  async awardStars(
    ...
    try {
      await this.db
        ...
        .run();
      logger.info(
        `Awarded ${amount} stars to player ${playerId} for reason: ${reason}`
      );
    } catch (error) {
  ...
  async deductStars(
    ...
    try {
      await this.db
        ...
        .run();
      logger.info(
        `Deducted ${amount} stars from player ${playerId} for reason: ${reason}`
      );
    } catch (error) {
  ...
  async getStarsBalance(playerId: number): Promise<number> {
    try {
      ...
      logger.error(
        `Failed to get stars balance for player ${playerId}:`,
        error
      );
