...
describe('EventBus Performance', () => {
  ...
  it('should handle wildcard listeners efficiently', () => {
    ...
    console.log(`Processed ${wildcardCounter} events across ${eventTypes.length} types in ${duration.toFixed(2)}ms`);
  });
  ...
  it('should measure async vs sync performance', async () => {
    ...
    console.log(`Sync is ${(asyncDuration / syncDuration).toFixed(2)}x faster for immediate processing`);
  });
});